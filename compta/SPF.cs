...
public class SPF : ICustomFunction, IFunction
{
	private const string functionName = "SPF";
	...
	ParameterValue IFunction.Evaluate(IList<ParameterValue> parameters, EvaluationContext context)
	{
		...
		OleDbCommand oleDbCommand = new OleDbCommand($"SELECT SUM(DEB)-SUM(CRE) FROM Balance   WHERE (EXERCICE={monModule.gExercice - 1}) AND  CPT LIKE '{cpt}%'", monModule.gbase);
		object o = oleDbCommand.ExecuteScalar();
		oleDbCommand.Dispose();
		return (o == DBNull.Value) ? 0m : Math.Round(Math.Abs(Convert.ToDecimal(o)), 0, MidpointRounding.AwayFromZero);
	}

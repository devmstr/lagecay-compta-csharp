...
public class NUMAI : ICustomFunction, IFunction
{
	private const string functionName = "NUMAI";

	private readonly ParameterInfo[] functionParameters;

	public string Name => "NUMAI";
	...
	{
		OleDbCommand oleDbCommand = new OleDbCommand("SELECT NUMAI  FROM Dossiers  WHERE  UNI='" + monModule.gUNI + "'", monModule.gbase);

...
public class EcrtriTableAdapter : Component
{
	...
	private void InitAdapter()
	{
		...
		tableMapping.DataSetTable = "Ecrtri";
		...
		_adapter.InsertCommand.CommandText = "INSERT INTO `Ecrtri` (`UNI`, `Exercice`, `JA`, `NOP`, `LIG`, `CPT`, `TRS`, `DAT`, `LIB`, `DEB`, `CRE`, `PTG`, `PTGX`, `RAP`, `Jour`, `UserC`, `DateC`, `UserM`, `DateM`, `ID`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		_adapter.InsertCommand.Parameters.Add(new OleDbParameter("ID", OleDbType.Integer, 0, ParameterDirection.Input, 0, 0, "ID", DataRowVersion.Current, sourceColumnNullMapping: false, null));
	}
	...
	private void InitCommandCollection()
	{
		...
		_commandCollection[0].CommandText = "SELECT UNI, Exercice, JA, NOP, LIG, CPT, TRS, DAT, LIB, DEB, CRE, PTG, PTGX, RAP, Jour, UserC, DateC, UserM, DateM, ID, NUM FROM Ecrtri";
	...
	public virtual int Fill(DataSet1.EcrtriDataTable dataTable)
	{
	...
	public virtual DataSet1.EcrtriDataTable GetData()
	{
		Adapter.SelectCommand = CommandCollection[0];
		DataSet1.EcrtriDataTable dataTable = new DataSet1.EcrtriDataTable();
	...
	[GeneratedCode("System.Data.Design.TypedDataSetGenerator", "16.0.0.0")]
	[HelpKeyword("vs.data.TableAdapter")]
	public virtual int Update(DataSet1.EcrtriDataTable dataTable)
	{
	...
	{
		return Adapter.Update(dataSet, "Ecrtri");
	}
	...
	public virtual int Insert(string UNI, int? Exercice, string JA, string NOP, int? LIG, string CPT, string TRS, DateTime? DAT, string LIB, decimal? DEB, decimal? CRE, string PTG, string PTGX, string RAP, byte? Jour, string UserC, DateTime? DateC, string UserM, DateTime? DateM, int? ID)
	{
		if (UNI == null)
		{
		...
		if (ID.HasValue)
		{
			Adapter.InsertCommand.Parameters[19].Value = ID.Value;
		}
		else
		{
